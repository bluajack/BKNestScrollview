...
#import "BKNestScrollview.h"
#import "WZBSegmentedControl/WZBSegmentedControl.h"
#import "BKBaseScrollview.h"
#import "BKHeadView.h"
#import "SURefresh/SURefreshHeader.h"
...
@interface BKNestScrollview ()<UIScrollViewDelegate>

@property (nonatomic, strong) WZBSegmentedControl *titleView;
@property (nonatomic, strong) BKHeadView *contentHeadView;
@property (nonatomic, strong) BKBaseScrollview *contentScrollview;
...
@implementation BKNestScrollview
...
+ (instancetype)nestScrollviewWithTitleArr:(NSArray<NSString*> *)titleArr
    ...
    BKNestScrollview *nestView = [[BKNestScrollview alloc] initWithFrame:frame];
...
+ (instancetype)nestScrollviewWithTitleArr:(NSArray<NSString*> *)titleArr
    ...
    BKNestScrollview *nestView = [self nestScrollviewWithTitleArr:titleArr SubVCArr:subVCArr Frame:frame];
...
//    __weak BKNestScrollview *weakSelf = self;
//    [self.contentScrollview addRefreshHeaderWithHandle:^{
//        for (BKNestBaseController *vc in weakSelf.subVCArr) {
//            [vc refreshHandle];
//        }
//    }];
    ...
    for (BKNestBaseController *vc in _subVCArr) {
...
- (BKBaseScrollview *)contentScrollview {
    if (_contentScrollview == nil) {
        _contentScrollview = [[BKBaseScrollview alloc] initWithFrame:self.bounds];
...
- (BKHeadView *)contentHeadView {
    if (_contentHeadView == nil) {
        _contentHeadView = [[BKHeadView alloc] initWithFrame:CGRectMake(0, 0, selfW, _contentHeadViewH)];
...
- (WZBSegmentedControl *)titleView {
    if (_titleView == nil) {
        __weak BKNestScrollview *weakSelf = self;
        _titleView = [WZBSegmentedControl segmentWithFrame:(CGRect){0, _headViewH, selfW, _titleViewH} titles:_titleArr tClick:^(NSInteger index) {
